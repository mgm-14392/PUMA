...
################################### CHEMICAL SPACE #########################################  
   ...
   output$plot <-renderPlotly({
     ...
     # we will call the columns be name so we rename them
     colnames(t_data) <- c("SMILES","DB","ID")
     
     # we reorder so compunds form same database are grouped
     ...
     names(descriptors)<-c("MW","TopoPSA","nRotB","nHBDon","nHBAcc","ALogP")
    ...
    if (NCOL(pca$x) == 6){
      results <- cbind(t_data, pca$x[,1:6])
    } else if (NCOL(pca$x) == 5){
      results <- cbind(t_data, pca$x[,1:5])
    } else if (NCOL(pca$x) == 4){
      results <- cbind(t_data, pca$x[,1:4])
    }
    ...
    # input index to choose column to plot
    PCa <- results[,as.numeric(input$PC1)]
    PCb <- results[,as.numeric(input$PC2)]
    PCc <- results[,as.numeric(input$PC3)]
...
###################################  PROPERTIES SUMMARY  #####################################   
   ...
   output$plot2 <-renderPlotly({
     ...
     descs2 <- select(descs, -one_of(c("SMILES", "ID")))
     
     ######################## SUMMARY 
     
     if(as.numeric(input$PROP) == 4){
       b <-descs2 %>% split(.$DB) %>% map(~summary(.$MW))} 
     else if(as.numeric(input$PROP) == 5){
       b <-descs2 %>% split(.$DB) %>% map(~summary(.$TopoPSA))} 
     else if(as.numeric(input$PROP) == 6){
       b <-descs2 %>% split(.$DB) %>% map(~summary(.$nRotB))} 
     else if(as.numeric(input$PROP) == 7){
       b <-descs2 %>% split(.$DB) %>% map(~summary(.$nHBDon))} 
     else if(as.numeric(input$PROP) == 8){
       b <-descs2 %>% split(.$DB) %>% map(~summary(.$nHBAcc))} 
     else if(as.numeric(input$PROP) == 9){
     ...
     Molecular.Property <- descs[,as.numeric(input$PROP)]
...
################################################ CSR curves ########################################################
   ...
   output$plot3 <- renderPlotly({
     ...
     if((NROW(UN)==1 || !is.list(FREQUNSCAF))){
       FREQUNSCAF2 <- FREQUNSCAF
     }else{
       FREQUNSCAF2 <- plyr::ldply(FREQUNSCAF, rbind)
     }
     
     if((NROW(UN)==1 || !is.list(FREQUNSCAF))){
       FREQUNSCAF2 <- FREQUNSCAF2
     }else{
       FREQUNSCAF2 <-as.data.frame(t(FREQUNSCAF2)) 
     }
     
     if((NROW(UN)==1 || !is.list(FREQUNSCAF))){
       FREQUNSCAF2 <- FREQUNSCAF2
     }else{
       colnames(FREQUNSCAF2) = as.character(unlist(FREQUNSCAF2[1,])) 
     }
     
     if((NROW(UN)==1 || !is.list(FREQUNSCAF))){
       FREQUNSCAF2 <- FREQUNSCAF2
     }else{
       FREQUNSCAF2 = FREQUNSCAF2[-1, ]  
     }
     
     if((NROW(UN)==1 || !is.list(FREQUNSCAF))){
       FREQUNSCAF2 <- FREQUNSCAF2
     }else{
       FREQUNSCAF2 <- apply(FREQUNSCAF2, 2, as.character) 
     }
     
     if((NROW(UN)==1 || !is.list(FREQUNSCAF))){
       FREQUNSCAF2 <- FREQUNSCAF2
     }else{
       FREQUNSCAF2 <- apply(FREQUNSCAF2, 2, as.numeric)  
     }
     ################### NUMBER OF UNIQUE SCAFFOLDS  #############################
     
     unique.scaf <- as.data.frame(apply(FREQUNSCAF2, 2, function(x){NROW(na.omit(x))})) #NUMBER OF SCAFFOLDS
     srt.by.freq <- apply(FREQUNSCAF2,2,sort,decreasing=T) #SORT BY FREQUENCY LIST#
     
     if((NROW(UN)==1 || !is.list(FREQUNSCAF))){
       srt.by.freq <- srt.by.freq
     }else{
       max.length <- max(sapply(srt.by.freq, length))
     }
     
     if((NROW(UN)==1 || !is.list(FREQUNSCAF))){
       srt.by.freq <- srt.by.freq
     }else{
       srt.by <- lapply(srt.by.freq, function(v) { c(v, rep(NA, max.length-length(v)))})
     }
     
     if((NROW(UN)==1 || !is.list(FREQUNSCAF))){
       srt.by.freq2 <- srt.by.freq
     }else{
       srt.by2 <- do.call(rbind, srt.by)
     }
     
     if((NROW(UN)==1 || !is.list(FREQUNSCAF))){
       srt.by.freq2 <- srt.by.freq
     }else{
       srt.by.freq2 <-as.data.frame(t(srt.by2))
     }
     ...
     uniq <-FREQUNSCAF2 #SELECT UNIQUE SCAFFOLDS#
     ...
     SING <- apply(FREQUNSCAF2, 2, function(x){sum(na.omit(x==1))})
...
output$plot4 <- renderPlotly({ 
  ...
  if((NROW(UN)==1) || num.comp[1,2]==num.comp[2,2]){
    ...
    uno <- NROW(unique(na.omit(Scf2[,1])))
  }
  
  if((NROW(UN)==1) || num.comp[1,2]==num.comp[2,2]){
    Scf2 <- Scf2
  }else {
    dos <- NROW(unique(na.omit(Scf2[,2])))
  }
  
  if((NROW(UN)==1) || num.comp[1,2]==num.comp[2,2]){
    frscaf <- sort(table(Scf2),decreasing=TRUE)
  } else if (uno == dos) {
    frscaf <- as.data.frame(apply(na.omit(Scf2),2,count))
  } else {
  ...
  if((NROW(UN)==1) || num.comp[1,2]==num.comp[2,2]){
    ...
    frscaf <- apply(frscaf,2,function(x){sort(x+1, decreasing = TRUE)})
  } else {
  ...
  if((NROW(UN)==1) || num.comp[1,2]==num.comp[2,2]){
    ...
    frscaf2 <- apply(frscaf,2,function(x){sort(x+1, decreasing = TRUE)})
  } else {
    frscaf <- frscaf 
  } 
  ...
  if((NROW(UN)==1) || num.comp[1,2]==num.comp[2,2]){
    ...
    frscaf2 <- frscaf2[,-seq(2, NROW(frscaf2), by = 2)]
  } else {
    frscaf <- frscaf 
  }
  ...
  if((NROW(UN)==1) || num.comp[1,2]==num.comp[2,2]){
    ...
    firsten <- as.data.frame(frscaf2[1:10,]) 
  } else {
  ...
  if((NROW(UN)==1) || num.comp[1,2]==num.comp[2,2]){
    ...
    firstwty <- as.data.frame(frscaf2[1:20,]) 
  } else {
  ...
  if((NROW(UN)==1) || num.comp[1,2]==num.comp[2,2]){
    ...
    firstrty <- as.data.frame(frscaf2[1:30,]) 
  } else {
  ...
  if((NROW(UN)==1) || num.comp[1,2]==num.comp[2,2]){
    ...
    firstfrty <- as.data.frame(frscaf2[1:40,]) 
  } else {
  ...
  if((NROW(UN)==1) || num.comp[1,2]==num.comp[2,2]){
    ...
    firstffty <- as.data.frame(frscaf2[1:50,]) 
  } else {
  ...
  if((NROW(UN)==1) || num.comp[1,2]==num.comp[2,2]){
    ...
    firstsxty <- as.data.frame(frscaf2[1:60,]) 
  } else {
...
if(length(nam.comp)==1){
compounds <- as.data.frame(compounds[,-1])
} else {
